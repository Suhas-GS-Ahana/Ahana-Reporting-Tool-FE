...
const saveProcess = async () => {
  ...
  const createTableMappings = async (
    ...
    const tableMappings = await Promise.all(
      createTableData.map(async ({ schema_name, table_name, dest_schema_name }) => {
            ...
            if (step.step_type === "import") {
              ...
              return {
                steps: {
                  ...
                  p_dest_data_source_id: Number(step.destination_connection_id),
                ...
                table_config: createTableData.map(
                  ({ schema_name, table_name, dest_schema_name }) => [
                    ...
                    // Destination table config
                    {
                      p_inserted_by: 1,
                      p_modified_by: 1,
                      p_connection_name: step.destination_details.connection_name,
