...
export default function CreateProcess() {
  ...
  //to fetch details of connection (/connection-view)
  ...
  const saveProcess = async () => {
      ...
      try {
        ...
        if (result.status === "success") {
          return result.data.map((column, index) => ({
            ...
            p_isautogenerated: isAutoGenerated(column.column_name),
            p_ordinal_position: index + 1,
            p_nullable: isNullable(column.data_type),
          }));
        }
        return [];
      } catch (error) {
    ...
    // Helper functions for data type mapping and column properties
    ...
    // };
    // const getColumnLength = (dataType) => {
    //   const match = dataType.match(/\((\d+)\)/);
    //   return match ? parseInt(match[1]) : 0;
    // };
    // const getColumnPrecision = (dataType) => {
    //   return 0;
    // };
    // const getColumnScale = (dataType) => {
    //   return 0;
    // };
    ...
    //   );
    // };
    const isAutoGenerated = (columnName) => {
      return (
        columnName.toLowerCase().includes("id") ||
        columnName.toLowerCase().includes("guid")
      );
    };
    const isNullable = (dataType) => {
          ...
          steps: await Promise.all(
            sp.steps.map(async (step) => {
              if (step.step_type === "import") {
                // Fetch column details for each table (only for import steps)
                const tableConfigDetails = await Promise.all(
                  step.selected_tables.map(
                    async ({ schema_name, table_name }) => {
                      // Fetch columns for source table
                      const sourceColumns = await fetchColumnDetails(
                        connectionId,
                        schema_name,
                        table_name
                  ...
                  table_config: step.selected_tables.map(
                    ({ schema_name, table_name }) => [
                      ...
                      },
                      {
                        ...
                        p_connection_name: connectionsDetails.connection_name,
                        p_database_name: connectionsDetails.database_name,
                        p_schema_name: destinationSchema,
                        p_table_name: table_name,
                        p_is_temp_table: false,
                        p_table_type: "staging",
                        p_table_category: "sales",
