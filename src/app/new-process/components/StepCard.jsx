...
const StepCard = ({
  ...
  const c_conn_id = searchParams.get("connectionName");
  ...
  const [selectedTables, setSelectedTables] = useState(
    currentStepState.selectedTables
  );
  const [expandedTables, setExpandedTables] = useState(
    currentStepState.expandedTables
  );
  const [selectedDestinationTables, setSelectedDestinationTables] = useState(
    currentStepState.selectedDestinationTables
  );
  const [expandedDestinationTables, setExpandedDestinationTables] = useState(
    currentStepState.expandedDestinationTables
  );
  const [destinationSchemaDetails, setDestinationSchemaDetails] = useState(
    currentStepState.destinationSchemaDetails
  );
  const [destinationTableDetails, setDestinationTableDetails] = useState(
    currentStepState.destinationTableDetails
  );
  const [selectedDestinationSchema, setSelectedDestinationSchema] = useState(
    currentStepState.selectedDestinationSchema
  );
  const [selectedDestinationConnection, setSelectedDestinationConnection] =
    useState(currentStepState.selectedDestinationConnection);
  const [destinationConnectionDetails, setDestinationConnectionDetails] =
    useState(currentStepState.destinationConnectionDetails);
  const [selectedSchema, setSelectedSchema] = useState(
    currentStepState.selectedSchema
  );
  const [tableDetails, setTableDetails] = useState(
    currentStepState.tableDetails
  );
  ...
  useEffect(() => {
    if (previousType !== step.type) {
      ...
      setSelectedDestinationConnection(
        newTypeState.selectedDestinationConnection
      );
      setDestinationConnectionDetails(
        newTypeState.destinationConnectionDetails
      );
    ...
    setSelectedTables((prev) => {
      ...
      if (!newSelection[tableName]) {
        ...
        if (newSelection[tableName].includes(columnName)) {
          ...
          newSelection[tableName] = newSelection[tableName].filter(
            (col) => col !== columnName
          );
  ...
  const toggleDestinationTableSelection = (tableName) => {
    setSelectedDestinationTables((prev) => {
      ...
      if (!newSelection[tableName]) {
        // If table wasn't selected, select it with all columns
        const tableData = destinationTableDetails.find(
  ...
  const handleLocalSchemaSelect = async (schema) => {
    ...
    try {
      ...
      console.log(c_conn_id);

      const response = await fetch(
        `${baseURL}/connection-tables?conn_id=${c_conn_id}&schema_name=${schema.schema_name}`
      );
      if (!response.ok) {
  ...
  const handleDestinationSchemaSelect = async (schema) => {
    ...
    try {
      const response = await fetch(
        `${baseURL}/connection-tables?id=${c_conn_id}&schema_name=application`
      );
      if (!response.ok) {
        throw new Error("Failed to fetch tables");
      }
  ...
  const handleDestinationConnectionSelect = async (dataSourcesId) => {
    ...
    try {
      console.log(dataSourcesId);
      
      const response = await fetch(
        `${baseURL}/connection-view?id=${dataSourcesId}`
      );
      if (!response.ok) {
