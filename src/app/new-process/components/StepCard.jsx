...
const StepCard = ({
  ...
  // Check if all columns of a table are selected
  const areAllColumnsSelected = (tableName) => {
    if (!selectedTables[tableName] || !tableDetails) return false

    const tableData = tableDetails.find((t) => t.table_name === tableName)
    if (!tableData) return false

    const columns = parseColumns(tableData.columns_with_types)
    return columns.length === selectedTables[tableName].length
  }
